...
class MiniMaxProvider(LLMProvider):
    """
    MiniMax использует OpenAI-compatible chat completions API.
    """
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not config.api_key:
            raise ValueError("MiniMax api_key is required")
    ...
    async def complete(
        ...
        async def _call() -> LLMResponse:
            ...
            if response.status_code >= 400:
                raise ProviderError(f"MiniMax error: {response.status_code} {response.text}")
            data = response.json()
        ...
        return await with_retry(
            ...
            operation_name=f"minimax/{model}",
        )
    ...
    async def stream(
        ...
        async with self._client.stream(
            ...
            if response.status_code >= 400:
                ...
                raise ProviderError(
                    f"MiniMax error: {response.status_code} {body.decode(errors='ignore')}"
                )
